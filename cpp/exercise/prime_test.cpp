...
void get_primes(std::vector<int> & output, const int max)
{
    ...
    std::vector<char> sieve((max - 1) / 2, 1);
    int i = 3;
    ...
    // will not affect the result. This is basically slightly modified 
	// classical implementation of sieve of Eratosthene. It's time complexity
	// is max * log log (max) instead of max * max
    while (i <= max / i)
	{
    ...
    int sieve_idx = calc_sieve_idx(i);
	while (i <= max)
	{
		if (sieve[sieve_idx])
		{
...
TEST(prime_test, extreme)
{
	...
	for (size_t i = 0; i < primesMax.size(); i++)
	{
		EXPECT_TRUE(is_prime(primesMax[i]));
	}
}
